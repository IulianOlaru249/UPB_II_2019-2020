...
%%myConcat(List, List, []).
%%myConcat([Head|Rest], List, [Head|Tail]) :-
%%    myConcat(Tail, List, Rest).

%lista concatenata cu lista vida este egala cu ea insasi
myConcat([], List, List).
%Lista concatenata cu primul element din coada este primul element din restul 
%listei daca Lista concatenata cu coada este restul listei.
myConcat([Head|Tail], List, [Head|Rest]) :-
    myConcat(Tail, List, Rest).
...
%lista vida este reveersul listei vide
myReverse([],[]).

%primul element din lista este reversul cozii daca coada este reversul restului listei si
%restul listei concatenata la lista formata din primul element este lista originala.
myReverse([Head|Tail], List):-
    myReverse(Tail, Rest),
