...
arithmetic a ratio = iterate (+ ratio) a
geometric :: Num a => a -> a -> [a]
geometric a ratio = iterate ( * ratio) a
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms x =  [(fromIntegral . fst) y | y <- iterate (next . snd) (next $ mkStdGen x)]
...
wordToAlpha x = let i = fromIntegral $ mod x 26 in chr $ ord 'a' + i
...
--randomAlphaKey x = [wordToAlpha y | y <- randoms x]
...
--tableToFunc table char = p where p = fst $ (filter (\x -> chr == (fst x)) table)
