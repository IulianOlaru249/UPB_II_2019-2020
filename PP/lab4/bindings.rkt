...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((seg (get-line-segment))
         (start_point (get-start-point seg))
         (end_point (get-end-point seg)))
    (sqrt (+ (* (- (car start_point) (car end_point)) (- (car start_point) (car end_point))) (* (- (cdr start_point) (cdr end_point)) (- (cdr start_point) (cdr end_point)))))))
...
(define (distance p1 p2)
  (compute-length (lambda () (cons p1 p2)) (lambda (seg) (car seg)) (lambda (seg) (cdr seg)) ))
...
(define (compute-perimeter points)
  (let iter ((points_aux points)(result 0))
    (if (null? (cdr points_aux))
        (+ result (distance (car points_aux) (car points)))
        (iter (cdr points_aux) (+ result (distance (car points_aux) (cadr points_aux)))))
    ))
...
;;(cons (list 1) (list 2))
;;(car (cons (list 1) (list 2)))
;;(cdr (cons (list 1) (list 2)))
;;(let ((a 5))
;;  (let ((f (lambda (b) (cons a b))))
;;    (let ((a 2) (b 5))
;;      (f 2))))
