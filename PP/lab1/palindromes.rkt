...
(define (nr-of-bases-palindrome n Bases)
  (if (null? Bases)
      0
     (if (palindrome? (num->base n (car Bases)))
         (+ 1 (nr-of-bases-palindrome n (cdr Bases)))
      (nr-of-bases-palindrome n (cdr Bases))
  )))

(nr-of-bases-palindrome 121 '(2 3 4 5 6 7 8 9 10))
   
(define (first-b-pal start b)
  (if (<= b (nr-of-bases-palindrome start '(2 3 4 5 6 7 8 9 10)))
      start
...
(define (longest-palindrome n)
  (if (palindrome? (list n))
      (list n)
      (if (>= (length(drop (list n) 1)) (length (take (list n) 1)))
          (list->num (drop (list n) 1))
        (list->num (take (list n) 1))
       )
      )
