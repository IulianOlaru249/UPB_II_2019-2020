...
(define-struct bird (y y_speed))
(define-struct pipe (x y))
(define-struct variabels (gravity momentum scroll-speed))
(define-struct game (bird pipes score variabels abilities))
;TODO 8
...
(define (get-initial-state)
  (game (bird bird-initial-y 0)
        (list (pipe scene-width (+ added-number (random random-threshold))))
        0
        (variabels initial-gravity initial-momentum initial-scroll-speed)
        null
        ))
...
; gravitația.
(define (next-state-bird bird1 gravity)
    (struct-copy bird bird1 [y (+ (get-bird-y bird1) (bird-y_speed bird1))] [y_speed (+ (bird-y_speed bird1) gravity)]))
...
(define (next-state-bird-onspace bird1 momentum)
  (struct-copy bird bird1 [y (get-bird-y bird1)] [y_speed (- momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy game current-state [bird (next-state-bird-onspace (get-bird current-state) initial-momentum)])
       current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (pipe1)
       (struct-copy pipe pipe1 [x (- (get-pipe-x pipe1) scroll-speed)] [y (pipe-y pipe1)]))
                               pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (append pipes (list (pipe
                           (+ (get-pipe-x (car (reverse pipes))) pipe-width pipe-gap)
...
(define (check-ground-collision bird)
 (if (>= (+ bird-height (get-bird-y bird)) ground-y)
     #t
     #f))
  ...
  (if (null? (filter (λ (pipe)
             (or
              (check-collision-rectangles (make-posn bird-x (get-bird-y bird))
                              ...
                              (make-posn (+ (get-pipe-x pipe) pipe-width) (pipe-y pipe)))
              (check-collision-rectangles (make-posn bird-x (get-bird-y bird))
                              (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height))
                              (make-posn (get-pipe-x pipe) (+ (pipe-y pipe) pipe-self-gap))
                              (make-posn (+ (get-pipe-x pipe) pipe-width) (+ (+ (pipe-y pipe) pipe-self-gap) pipe-height)))))
             pipes))
      #f
     #t))
...
(define (next-state state)
  (struct-copy game state
               ...
               [pipes (next-state-pipes (get-pipes state) initial-scroll-speed)]
               [score (+ (get-score state) 0.1)]
               ))
...
;(place-image 'image 'x_mid 'y_mid 'bottom_image)
(define (draw-frame state)
  (place-image bird-image (+ bird-x (quotient bird-width 2)) (+ (get-bird-y (get-bird state)) (quotient bird-height 2))
                (place-image ground-image (quotient scene-width 2) (quotient (+ scene-height ground-y) 2)
...
(define (place-pipes pipes scene)
	(if (null? pipes)
            scene
            (place-pipes (cdr pipes) (place-image pipe-image
                                                  (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                                                  (- (pipe-y (car pipes)) (quotient pipe-height 2))
                                                  (place-pipes (cdr pipes) (place-image pipe-image
                                                                                        (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                                                                                        (+ (+ (pipe-y (car pipes)) pipe-self-gap) (quotient pipe-height 2))
                                                                                        scene))))))
...
(define fast-ability
  (ability (hourglass "mediumseagreen") 10 null (λ () ('a))))

; Abilitatea care va încetini timpul va dura 30 de secunde, va avea imaginea (hourglass "tomato")
; va avea inițial poziția null si va modifica scrolls-speed dupa formulă
; scroll-speed = scroll-speed + 1
(define slow-ability
  (ability (hourglass "tomato") 30 null (λ () ('a))))

; lista cu toate abilităţile posibile în joc
(define ABILITIES (list slow-ability fast-ability))

(define get-variables (λ (state) (game-variabels state)))
(define get-variables-gravity (λ (variabels) (variabels-gravity variabels)))
(define get-variables-momentum (λ (variabels) (variabels-momentum variabels)))
(define get-variables-scroll-speed (λ (variabels) (variabels-scroll-speed variabels)))
...
(define (get-abilities x)
  (cons
   (filter (λ (ability)
             (not (check-collision-rectangles (make-posn bird-x (get-bird-y bird))
                              (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height))
                              (make-posn (- (posn-x (get-ability-pos ability)) (quotient image-width 2)) (- (posn-y (get-ability-pos ability)) (quotient image-height 2)))
                              (make-posn (+ (posn-x (get-ability-pos ability)) (quotient image-width 2)) (+ (posn-y (get-ability-pos ability)) (quotient image-height 2))))))
           (game-abilities x))
   (filter (λ (ability)
             (check-collision-rectangles (make-posn bird-x (get-bird-y bird))
...
(define (move-abilities abilities scroll-speed) 'no)
