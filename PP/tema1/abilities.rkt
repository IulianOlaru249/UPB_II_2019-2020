...
(define (position-abilities abilities)
	(append                                                                           ;combine updated abilities with old abilities
          (map (λ (nul-ability)                                                           ;update their position
               (struct-copy ability nul-ability [image (get-ability-image nul-ability)]
                                                [time (get-ability-time nul-ability)]
                                                [pos (random-position POSITION_RANGE)]
                                                [next (get-ability-next nul-ability)]))
                     (filter (λ (abil)                                                    ;get all abilities with pos null
                               (null? (get-ability-pos abil))
                               abilities)))
         (filter (λ (abil)                                                                ;get all other abilities (with pos != null)
                    (not (null? (get-ability-pos abil)))
                      abilities))))
...
(define (compose-abilities L)
	(apply compose
               (foldr (λ (elem acc)
                        (cons acc  (get-ability-next elem))) L)))
