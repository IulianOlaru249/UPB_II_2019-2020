...
class (Ord a) => PQueue pq a where
    ...
    -- inserarre in coada goala de la dreapta la stanga
    fromList = foldr insert empty

    toList :: pq a -> [(Prio, a)]
    toList q = if isEmpty q 
                then [] 
                else (fromJust $ top q) : (toList $ pop q)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert el (LPQ q) = if null q
                    --construieste lista noua cu elementul
                    then LPQ [el]
                    -- construieste o lista noua cu primul element si restul listei
                    else if fst el >= fst (head q) then LPQ $ el : q
                                        -- construieste o lista noua cu prima parte din lista parametru
                                        -- si lista rezultata din inserarea elementului inainte de primul element din restul listei
                                        -- cu prioritate mai mica decat el.
                                        else LPQ $ (head q) : toList (insert el  (LPQ $ tail q))

    top (LPQ q) = if null q then Nothing else Just $ head q

    --construieste lista noua din vechea lista mai putin primul element din lista veche
    pop (LPQ q) = LPQ $ tail q

    isEmpty (LPQ q) = null q
...
--verific radacina si vad care are prioritatea mai mare. Acela devine noua radacina. Pastrezi subarborele stang. 
--Se face merge intre subarborele drept cu subarborele t2
--dintre subarbori, cel cu rankul mai mare va sta in stanga
merge (Empty a) (Empty b) = (Empty 0)
merge (Empty _) t2 = t2
merge t1 (Empty _) = t1
merge (Node rank1 nodeVal1 left1 right1) (Node rank2 nodeVal2 left2 right2) =  if (fst nodeVal1) <= (fst nodeVal2) then merge (Node rank2 nodeVal2 left2 right2) (Node rank1 nodeVal1 left1 right1)
                                                                                else if (rank left1) >= (rank merged)
                                                                                    then (Node ((rank merged) + 1) nodeVal1 left1 merged)
                                                                                    else (Node ((rank left1) + 1) nodeVal1 merged left1)
                                                                                where merged = (merge right1 (Node rank2 nodeVal2 left2 right2))

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node rank root left_child right_child) = (inorder left_child) ++ [root]
                                                ++ (inorder right_child)
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    insert val t1 = merge t1 (Node 0 val (Empty 0) (Empty 0))

    top (Empty _) = Nothing
    top (Node rank root left_child right_child) = Just root

    pop (Empty _) = (Empty 0)
    pop r = merge (left r) (right r)
