...
primes :: Int -> [Int]
primes n = [j | j <- [2, 3 .. n], all (\k -> if (j `mod` k /=0) then True else False)
                                           (primes (floor (sqrt (fromIntegral j))))]
...
setDiff :: Eq a => [a] -> [a] -> [a]
setDiff a b = [x | x <- a , not ( x `elem` b )]

cartProduct :: [a] -> [b] -> [(a, b)]
cartProduct a b = [(x, y) | x <- a, y <- b] 

setUnion :: Eq a => [a] -> [a] -> [a]
setUnion a b = [x | x <- ( (setDiff a b)  ++ (setIntersection a b) ++ (setDiff b a))]
...
{-
This works and i get what it does but it from stack overflow 
and I don t really know how to change it :(
group2 (x:y) = group_loop [x] x xs
  where
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc list = [(head x,  length x ) | x <- group (sort list) ]
...
-}
dup :: String -> String
dup str = unwords [x ++ " " ++ x | x <- words str]
...
isIsogram str= if (length [ y | y <- nrOcc [toLower x | x <- str, isLetter x], (snd y) == 0] ) /= 0 then False else True
