...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
    ...
    ether_hdr->ether_dhost = eth_hdr(original)->ether_shost;
    ether_hdr->ether_dhost = original shost;
	...
	//while (i < count) {
		...
		ip_hdr->id = htons (ip_hdr->id(original));
		ip_hdr->frag_off = htons (0);
		ip_hdr->ttl = 64;
		ip_hdr->protocol = IPPROTO_ICMP; //8
		...
		icmp_hdr->un.echo.id = htons(icmphdr->id(original));
		...
		printf("sequence:%d ttl:%d\n", ricmp_hdr->un.echo.sequence, rip_hdr->ttl);
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	do {
		...
		icmp_hdr->un.echo.sequence = ip_hdr->ttl;
		icmp_hdr->checksum = 0;
		icmp_hdr->checksum = checksum(icmp_hdr, sizeof(struct icmphdr));
		...
		struct iphdr *rip_hdr = (struct iphdr *)(reply.payload + IP_OFF);
		ricmp_hdr = (struct icmphdr *)(reply.payload + ICMP_OFF);

        addr.s_addr = rip_hdr->saddr;
		printf("%d: %s\n", ip_hdr->ttl, inet_ntoa(addr));

	}while(ricmp_hdr->type == ICMP_TIME_EXCEEDED);
}
